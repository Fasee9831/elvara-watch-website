    ...
    >
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden flex items-center justify-center bg-[#070706]">
        ...
        <div className="scene-overlay-hud">
                ...
                return (
                  <div key={phase.id} className="flex items-center space-x-3">
                    <div className="flex flex-col space-y-1 items-center">
                      ...
                      <span
                        className={`font-metadata text-[0.65rem] tracking-wider text-center ${
                          isActive
          ...
          {/* Center-Right / Bottom Floating Dynamic Narrative Glass Card */}
          <div className="relative max-w-md self-end my-auto pointer-events-auto">
            {NOCTURNE_PHASES.map((phase, idx) => {
              const isActive = activePhaseIdx === idx;
              ...
              return (
                <div
                  key={phase.id}
                  className={`glass-panel-luxury p-4 sm:p-5 shadow-2xl transition-all duration-500 transform ${
                    isActive
                ...
                >
                  ...
                  <h3 className="font-cinzel text-lg sm:text-xl lg:text-2xl text-gold-bright font-light leading-tight mb-1 text-shadow-cinematic">
                    {phase.title}
                  </h3>

                  <p className="font-display italic text-xs lg:text-[0.82rem] text-[var(--color-accent)] mb-2">
                    {phase.subtitle}
                  </p>

                  <p className="font-body text-[0.7rem] sm:text-[0.76rem] text-[var(--color-text-secondary)] font-light leading-relaxed mb-3">
                    {phase.description}
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2 pt-0.5">
                    {phase.specs.map((spec, sIdx) => (
                      ...
                      >
                        ...
                        <span className="font-sans text-[0.65rem] sm:text-[0.72rem] text-[var(--color-text-primary)] font-medium">
                          {spec.value}
                        </span>
                      </div>
                    ))}
